        ...
        <el-row>
            <el-col :span="12">
                <el-form-item prop="subject_level" label="科目级别">
                    <el-select v-model="form.subject_level" class="w-200" disabled>
                ...
                <el-form-item prop="subject_code" label="科目代码">
                    <el-input type="text" v-model="form.subject_code" placeholder="请输入科目代码" clearable disabled class="w-200"></el-input>
                </el-form-item>
            ...
            <el-col :span="12">
                <el-form-item prop="parent_subject" label="父级科目">
                    <el-select v-model="form.parent_subject" disabled class="w-200">
                        <el-option v-for="(item, index) in parentSubjectArr" :key="index" :label="item.subject_code + item.subject_name" :value="item.subject_code">
...
<script>
    export default {
        ...
        methods: {
            commit() {
                this.$refs.form.validate(valid => {
                    if (valid) {
                        ...
                        api.post('subject/edit', param).then(() => {
                            this.$message.success('编辑会计科目成功');
                            this.closeDialog();
            ...
            initForm() {
                this.subjectLevelChange(this.theSubject.subject_level);
                this.form = this.theSubject;
            }
        },
        created() {
            this.initForm();
