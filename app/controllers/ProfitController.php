...
class ProfitController extends ControllerBase
{
     ...
     * 利润汇总报表
     */
    public function profitSummaryListAction()
    {
        $get = $this->request->get();
        $page = $get['page'] ?: 1;
        $pageSize = $get['pageSize'] ?: 10;
        ...
        $profit = new ProfitService();
        $result = $profit->getProfitSummaryList($where, $page, $pageSize);
     ...
     * 利润汇总报表导出
     */
    public function exportProfitSummaryAction()
    {
        $get = $this->request->get();
        ...
        $where['oper_type'] = '1';
        $profit = new ProfitService();
        $result = $profit->exportProfitSummary($where);

        $log = new OperateLogService();
        if (!$result['status']){
            $log->addOperateLog($this->user['user_id'], $this->user['account'], OperateLogAction::PROFITSUMMARYEXPORT, OperateLog::STATUS_FAILED);
            Result::error(ResultCode::DB_QUERY_FAIL, '导出失败');
        }

        $list = $result['data']['list'];
        $log->addOperateLog($this->user['user_id'], $this->user['account'], OperateLogAction::PROFITSUMMARYEXPORT, OperateLog::STATUS_SUCCESS);
